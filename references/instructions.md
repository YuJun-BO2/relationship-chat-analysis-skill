...
```
VAULT_ROOT = configured by the user or inferred from the current workspace
...
```bash
find "$VAULT_ROOT" -maxdepth 1 \( -name "2025*.md" -o -name "2026*.md" \) | sort
```
...
Read `references/extraction-prompt.md` to get the full prompt template.
...
Read `references/synthesis-prompt.md` to get the full prompt template.
